...
public class Group {
    ...
    public Group(String sender, String[] receivers){
        _sender = sender;
        _receivers = new LinkedList();
    ...
    public static Vector<Group> generateGroups(Vector<String> mails, int number) {
        if(number < 1){
        ...
        int index = 0;
        while (temp.size() > 0) {
            Group current = lists.elementAt(index);
